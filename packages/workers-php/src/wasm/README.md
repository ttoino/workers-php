...
**built from source** by `build/build-php-wasm.sh`; see
`../../../../build/README.md`.
...
| File | Origin | Notes |
|---|---|---|
| `php-web.wasm`              | built  | PHP 8.5 compiled with `MAIN_MODULE=0 ASYNCIFY=1`. ~33 MB. |
...
grep -c 'new WebAssembly\.Module('  packages/workers-php/src/wasm/php-web.mjs   # must be 0
grep -c 'loadDylibs'                packages/workers-php/src/wasm/php-web.mjs   # must be 0
grep -c 'dynamicLibraries'          packages/workers-php/src/wasm/php-web.mjs   # must be 0
```
...
```bash
npm run build-wasm    # produces *.staged next to the active files
npm run wasm-promote  # promotes *.staged → active, old → *.legacy
rm packages/workers-php/src/wasm/*.legacy      # once satisfied
```
